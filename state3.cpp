...
double mass0 = 938e-6; // Rest mass of the particle in GeV/c^2
...
PVV propagateByMultipleScattering(TVector3 &momentum, double distance, double X0)
{
    if (!X0) // X0 = 0 means X0 is infinity
...
vector<TVector3> propagate(double momentumz)
{
    ...
    for (int i = 0; i < numDetectors; i++)
    {
        ...
        auto prop_ = [&](double distance, double X0)
        {
            auto [d1, a1] = propagateInMagneticField(position, momentum, distance);
            auto [d2, a2] = propagateByMultipleScattering(momentum, distance, X0);
        ...
        prop_(3e-4, 9.4e-2);  // The first silicon detector (detector the particle's x position)
        prop_(1e-2, 18.8e-2); // The carbon support
        prop_(3e-4, 9.4e-2);  // The second silicon detector (detector the particle's y position)
        hits.push_back(position);

        // propagate in detector gaps
        prop_(detectorGap - 106e-4, 0); // The vacuum gap (0 means X0 is infinity)
    }
...
vector<PDD> positionMomentumResolution(vector<double> &momentums, double resolution)
{
    ...
    for (double momentum : momentums)
    {
        ...
        positionMomentumCanvas->Print("positionMomentum.png"); exit(0);
